...
- **参照**:
  ...
  - レビュー結果報告書（`review_detail_design_p3_v0.2.txt`。🔴0件・🟡0件・🟢3件）
...
- [x] **表に行を足したあと、その表を指す記述を追ったか** 🆕（v0.3追加・レビューDD2-02/DD2-03対応） → v0.2 は §6 に3行（#12〜#14）を足したが**§2-4-1 の番号参照が #12 のまま**だった。§1-7 を新設したが**§5-1 の参照先ファイル一覧に反映していなかった**。**同じ型が2件同時に出た**ため、§5-1 のA区分に「§6 への番号参照を全件突き合わせる」を追加した
...
| **12** 🆕 | **基本設計書の次版** | — | **DD-03**：**§1-2 のコンポーネント一覧に `App.jsx` を追加する**（レビューDD-R-01）。一覧11件は `App.jsx` を含んでおらず、**3文書（基本設計書§1-2・本書§1・テスト設計書§1-2-1）が同じ前提を共有していたため、どの工程でも検出されなかった**。DD-01／DD-02 と同じ扱い | ⬜ |
